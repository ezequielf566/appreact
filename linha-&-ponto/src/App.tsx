...
export default function App() {
  ...
  const analyzeImage = async () => {
    ...
    if (!apiKey) {
      setError("Chave de API não encontrada. Por favor, configure o GEMINI_API_KEY nas configurações.");
      return;
    }
    ...
    try {
      ...
      try {
        const outlinePrompt = "Crie um risco de bordado técnico e minimalista (line art). REGRAS OBRIGATÓRIAS: 1. FOQUE APENAS NA PESSOA OU OBJETO PRINCIPAL. 2. REMOVA COMPLETAMENTE O CENÁRIO E FUNDO. 3. PROIBIDO QUALQUER TEXTO OU LETRAS NA IMAGEM. 4. Apenas linhas pretas finas e nítidas sobre fundo 100% branco sólido. 5. Estilo: Desenho de contorno limpo para transferência em tecido, sem sombreamento.";
  ...
  const generateColorVersion = async () => {
    ...
    try {
      ...
      const colorPrompt = "Transforme a imagem em um BORDADO MANUAL REALISTA (Hand Embroidery). REGRAS OBRIGATÓRIAS: 1. FOQUE APENAS NA PESSOA OU OBJETO PRINCIPAL. 2. Fundo 100% branco sólido. 3. A imagem NÃO PODE parecer uma foto. Ela DEVE parecer uma arte feita inteiramente de FIOS DE LINHA GROSSOS (textura 3D de meadas de bordado). 4. Os olhos, pele e detalhes devem ser simplificados e construídos com PONTOS DE BORDADO VISÍVEIS (ponto cheio, matiz, nó francês), mostrando a direção das linhas. 5. Sem texturas fotográficas, apenas textura de fios de algodão brilhantes sobrepostos. 6. Cores sólidas simulando paleta de linhas DMC.";
                        ...
                        {(activeTab === 'outline' ? generatedOutlineImage : generatedColorImage) ? (
                          <div className="relative space-y-8">
                            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-2xl relative">
                              ...
                              {/* Image Display */}
                              {!showMockup ? (
                                <div className="relative group/image rounded-2xl overflow-hidden bg-stone-50 border border-stone-100">
                                  <img 
                                    src={(activeTab === 'outline' ? generatedOutlineImage : generatedColorImage) || ''} 
                                    alt="Risco de Bordado Final" 
                                    className="w-full max-h-[700px] object-contain"
                                  />
                                  <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover/image:opacity-100 transition-opacity">
                                    <button
                                      onClick={() => setIsFullscreen(true)}
                                      className="p-4 bg-white text-violet-600 rounded-full hover:scale-110 transition-transform shadow-xl"
                                      title="Ampliar Arte"
                                    >
                                ...
                                <div className="relative mx-auto w-full max-w-[450px] aspect-square">
                                  {/* Metal Clasp */}
                                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 w-12 h-8 bg-gradient-to-b from-stone-300 to-stone-400 rounded-t-lg z-10 border-2 border-stone-500 flex items-center justify-center shadow-lg">
                                    <div className="w-16 h-2 bg-stone-400 rounded-full absolute top-2 border border-stone-500"></div>
                                  </div>
                                  {/* Hoop */}
                                  <div className="w-full h-full rounded-full border-[16px] border-[#d4a373] shadow-[inset_0_0_20px_rgba(0,0,0,0.5),0_10px_30px_rgba(0,0,0,0.3)] overflow-hidden bg-white relative group/image flex items-center justify-center transition-all duration-300">
                                    ...
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/image:opacity-100 transition-opacity flex items-center justify-center gap-4">
                                      ...
                                      <button
                                        onClick={shareImage}
                                        className="p-4 bg-white text-violet-600 rounded-full hover:scale-110 transition-transform shadow-xl"
                                        title="Compartilhar (WhatsApp, Drive, etc)"
                                      >
                                        <Share2 className="w-6 h-6" />
                                      </button>
                                    </div>
